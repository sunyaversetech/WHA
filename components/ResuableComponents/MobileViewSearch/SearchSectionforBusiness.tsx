...
export default function MobileBusinessSearchWithDates({
  ...
  const triggerSummary =
    [service, location, selDay ? whenLabel() : ""]
      .filter(Boolean)
      .join(" · ") || "Search for a service...";
  ...
  /* ── step header label ── */
  const stepLabel = { what: "What service?", where: "Where?", when: "When?" }[
    step
  ];
  const stepIcon = {
    what: <Search size={18} color="#64748b" />,
    where: <MapPin size={18} color="#64748b" />,
    when: <Clock size={18} color="#64748b" />,
  }[step];
  const stepVal = {
    what: serviceQuery,
    where: location,
    when: selDay ? whenLabel() : "",
  }[step];
            ...
            <div
              ...
              <span
                style={{
                  fontSize: 14,
                  fontWeight: 600,
                  color: service || location ? "#1e293b" : "#94a3b8",
                  flex: 1,
                  overflow: "hidden",
      ...
      <DrawerContent
          ...
          <div
            ...
            {step === "what" ? (
              <input
                type="text"
                autoComplete="off"
                className="placeholder:text-gray-400 text-sm focus-within:outline-none focus-within:ring-0 focus-within:ring-offset-0"
                value={serviceQuery}
                onChange={(e) => {
        ...
        <div
          ...
          <button
            ...
            Search
            {service && (
              <span style={{ fontWeight: 400, opacity: 0.7, fontSize: 14 }}>
                · {service}
              </span>
            )}
          </button>
