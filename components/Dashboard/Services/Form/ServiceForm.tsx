...
const NAV_SETTINGS = [
  "Online booking", "Portfolio images", "Forms", "Commissions", "Settings",
] as const;
  ...
  const form = useForm<ServiceFormValues>({
    ...
    defaultValues: initialData
      ? {
          ...
          assigned_employees:         (initialData.assigned_employees ?? []).map((e: any) => (typeof e === "string" ? e : e._id)),
      ...
      <div className="flex flex-col md:flex-row flex-1">
        ...
        <div className="w-full md:w-64 shrink-0 p-4 md:p-6 border-b md:border-b-0 md:border-r border-gray-100">
              ...
              <button key={item.id} type="button" onClick={() => setSection(item.id)}
                ...
                {item.id === "team" && teamCount > 0 && (
                  <span className={cn(
                    ...
                    section === "team" ? "bg-white/20 text-white" : "bg-gray-100 text-gray-500",
                  )}>{teamCount}</span>
                )}
              </button>
            ))}
          ...
          <div className="border-t border-gray-100 pt-4">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest px-3 mb-2">Settings</p>
            <nav className="space-y-0.5">
              {NAV_SETTINGS.map((label) => (
                <button key={label} type="button"
                  className="w-full text-left px-3 py-2.5 rounded-xl text-sm text-gray-400 cursor-not-allowed">
                  {label}
                </button>
              ))}
            </nav>
                ...
                <>
                  <div>
                    ...
                    <FormField control={form.control} name="name" render={({ field }) => (
                      <FormItem className="mb-4">
                        ...
                        <FormControl>
                          <Input {...field} maxLength={255} placeholder="Add a service name, e.g. Men's Haircut"
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
                      ...
                      {/* Duration */}
                      <FormField control={form.control} name="base_duration" render={({ field }) => (
                        <FormItem>
                    ...
                    <div className="flex items-center gap-3">
                      <button type="button"
                        ...
                        <span className="text-[#051e3a] text-base font-bold leading-none">+</span>
                        Add extra time
                      </button>
                    </div>
                  </div>
                        ...
                        {allEmployees.map((emp: any, idx: number) => {
                          ...
                          return (
                            <label key={emp._id} className="flex items-center gap-3 cursor-pointer p-3 rounded-xl hover:bg-gray-50 transition-colors">
                              <button type="button" onClick={() => toggleEmployee(emp._id)}
                                className={cn(
                                  "w-5 h-5 rounded border flex items-center justify-center shrink-0 transition-colors",
