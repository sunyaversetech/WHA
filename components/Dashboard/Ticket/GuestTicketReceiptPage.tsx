...
import { loadGuestReceipt, type GuestReceiptTicket } from "@/lib/guestReceipt";
...
const NotFoundState = () => (
  <div className="min-h-screen max-w-lg mx-auto px-4 py-16 text-center flex flex-col items-center gap-4">
    ...
    <div>
      <p className="font-bold text-foreground">Receipt not available</p>
      <p className="text-sm text-muted-foreground mt-1">
        This confirmation page is only available right after checkout. Check
        your email — we&apos;ve sent a copy of your tickets and invoice there.
      </p>
    </div>
...
export default function GuestTicketReceiptPage() {
  const { id } = useParams<{ id: string }>();
  // undefined = still checking sessionStorage, null = nothing found there.
  const [item, setItem] = useState<GuestReceiptTicket | null | undefined>(
    undefined,
  );

  useEffect(() => {
    const timer = setTimeout(() => setItem(loadGuestReceipt(id)), 0);
  ...
  if (item === undefined) return <ReceiptSkeleton />;
  if (item === null) return <NotFoundState />;
  ...
  const holderName = item.holderName || "Ticket Holder";
  ...
  const buildTicketPdf = (logo: HTMLImageElement | null) => {
    ...
    codes.forEach((code, i) => {
      ...
      let y = 28;
      if (dateRange) {
        const dateLine = `${new Date(dateRange.from).toLocaleDateString("en-AU", { day: "numeric", month: "short", year: "numeric" })}`;
        ...
        <div className="rounded-2xl p-4 bg-emerald-50 border border-emerald-200 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-emerald-800">
              You&apos;re all set, {holderName}!
            </p>
            <p className="text-xs text-emerald-700 mt-0.5">
              Your payment was successful. Download your tickets and invoice
              below — this page won&apos;t be available again, but we&apos;ve
              also emailed you a copy.
            </p>
          </div>
        </div>
