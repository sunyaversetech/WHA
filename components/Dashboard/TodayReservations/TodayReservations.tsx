...
const STATUS_META: Record<string, { label: string; badge: string }> = {
  ...
  confirmed:   { label: "Confirmed",   badge: "bg-blue-50 text-blue-700 border border-blue-200" },
  rescheduled: { label: "Rescheduled", badge: "bg-purple-50 text-purple-700 border border-purple-200" },
  arrived:     { label: "Arrived",     badge: "bg-teal-50 text-teal-700 border border-teal-200" },
  ...
  no_show:     { label: "No Show",     badge: "bg-gray-100 text-gray-600 border border-gray-200" },
};
          ...
          <div className="flex items-center gap-2 text-xs text-gray-500">
            ...
            <span className="truncate">
              ...
              {booking.total_price != null && (
                <span className="text-gray-400 ml-1">· ${booking.total_price}</span>
              )}
            </span>
          </div>
        )}
          ...
          {nextStatuses.map((st) => (
            <button
              ...
              className={cn(
                ...
                STATUS_ACTION_COLORS[st] ?? "bg-gray-600 hover:bg-gray-700 text-white",
              )}>
...
export default function TodayReservations() {
  ...
  const filtered = useMemo(() => {
    ...
    if (timeSlot !== "all") {
      list = list.filter((b) => getTimeSlot(new Date(b.start_time)) === timeSlot);
    }
    ...
    <div className="min-h-screen max-w-5xl mx-auto">
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-bold text-[#051e3a]">Today&apos;s Reservations</h1>
          </div>
      ...
      {!loading && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          <StatCard label="Total today"  value={stats.total}     color="bg-[#051e3a]" />
          <StatCard label="Confirmed"    value={stats.confirmed} color="bg-blue-400" />
          <StatCard label="Arrived"      value={stats.arrived}   color="bg-teal-400" />
          <StatCard label="Completed"    value={stats.completed} color="bg-emerald-400" />
        </div>
      )}
        ...
        {allStatuses.length > 1 && (
          <div className="relative shrink-0">
            <button
              ...
              {statusFilter === "all" ? "All statuses" : (STATUS_META[statusFilter]?.label ?? statusFilter)}
                ...
                <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg py-1.5 z-20 min-w-[160px]">
                  <button
                    ...
                    className={cn(
                      ...
                      statusFilter === "all" ? "font-semibold text-[#051e3a]" : "text-gray-600 hover:bg-gray-50",
                    )}>
                    All statuses
                  </button>
                  {allStatuses.map((st) => (
                    <button
                      ...
                      className={cn(
                        "w-full text-left px-4 py-2.5 text-sm transition-colors",
                        statusFilter === st ? "font-semibold text-[#051e3a]" : "text-gray-600 hover:bg-gray-50",
                      )}>
                      {STATUS_META[st]?.label ?? st}
                    </button>
                  ))}
      ...
      <div className="flex gap-2 flex-wrap mb-5">
        {TIME_SLOTS.map((ts) => (
          <button
            ...
            {ts.label}
            {ts.range && (
              <span className={cn("text-[11px] font-normal", timeSlot === ts.id ? "text-white/70" : "text-gray-400")}>
                {ts.range}
              </span>
            )}
          </button>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-200 py-20 text-center">
          ...
          {(search || timeSlot !== "all" || statusFilter !== "all") && (
            <button
              onClick={() => { setSearch(""); setTimeSlot("all"); setStatusFilter("all"); }}
        ...
        <>
          <p className="text-xs text-gray-400 mb-3">
            Showing {filtered.length} of {bookings.length} reservation{bookings.length !== 1 ? "s" : ""}
          </p>
          <div className="space-y-3">
